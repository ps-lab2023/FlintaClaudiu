...
function HotelsPageLogged2() {
  ...
  const [selectedValue, setSelectedValue] = useState("");
    ...
    function handleSearch() {
      navigate("/hotels", { state: { region: selectedValue, guests: noOfGuests } });
    }

  useEffect(() => {
    fetch(`http://localhost:8081/hotel/regionAndGuests?region=${region}&noMaxOfGuests=${guests}`)
  ...
  const handleInfoClick = (id) => {
    ...
    navigate("rooms", { state: { id: id } });
  };
  

  return (
    <div className="hotels-container">
      <h2>Hotels in {region}</h2>
...
export default HotelsPageLogged2;